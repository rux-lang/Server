...
use rux_manifest::{
    BuildMode, DefineValue, DependencySource, License, MANIFEST_MAX_BYTES, ManifestErrorCode,
    ManifestKind, Optimization, PackageType, ValidationProfile, parse_manifest,
...
"#;
    ...
    assert!(
        matches!(package.license(), Some(License::Expression(value)) if value == "MIT OR Apache-2.0")
    );
    assert_eq!(
