...
printf '#!/usr/bin/env sh\nexit 0\n' > "$fixture_root/bin/rux-playgroundd"
printf '#!/usr/bin/env sh\nexit 0\n' > "$fixture_root/bin/sqlx"
chmod 0755 "$fixture_root/bin/rux-server" "$fixture_root/bin/rux-playgroundd" "$fixture_root/bin/sqlx"
...
export RUX_RELEASE_PLAYGROUND_BINARY="$fixture_root/bin/rux-playgroundd"
