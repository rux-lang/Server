...
Production is a single hand-configured Ubuntu 26.04 LTS droplet running Caddy, PostgreSQL 18, and an unprivileged systemd API service. There is no provisioning automation: every production change is an operator action, written down in [deployment.md](deployment.md) rather than encoded in a playbook. Deploying replaces the installed binaries and restarts; migrations are applied as a separate, reviewed step and are never reverted. Administrator SSH, PostgreSQL, and the API all stay on loopback or behind the firewall, and only Caddy is exposed publicly.
