...
Production is a single hand-configured Ubuntu 26.04 LTS droplet running Caddy, PostgreSQL 18, and an unprivileged systemd API service. There is no provisioning automation: the host is built and recovered by operator action, written down in [deployment.md](deployment.md) and [recovery.md](recovery.md) rather than encoded in a playbook. Releasing is automated, and only releasing: pushing a `vX.Y.Z` tag whose commit is on `main` runs the pipeline in `.github/workflows/release.yml`, which rebuilds the gates, backs up the database, applies pending migrations, replaces the installed binaries, restarts, and verifies liveness and readiness — rolling the binaries back if it cannot. Migrations remain a distinct step within that run and are never reverted. Configuration is never deployed; `/etc/rux/config.toml` exists only on the host. Administrator SSH, PostgreSQL, and the API all stay on loopback or behind the firewall, and only Caddy is exposed publicly.

Logs are structured JSON on stdout and captured by journald, and they are the only telemetry channel — there is no metrics endpoint, no scrape target, and no separate observability stack. Each request logs its server-generated request ID, method, route template, status, and duration; the cleanup sweep and playground runs log their outcomes the same way. Route templates rather than concrete paths keep submitted values out of the logs. Encrypted pgBackRest backups and WAL archives go to a dedicated versioned Space in another region, package bucket versioning remains enabled, and both restore paths are rehearsed quarterly under the [production recovery contract](recovery.md).
