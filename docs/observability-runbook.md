...
## RuxServerPlaygroundUnavailable

Impact: `POST /v1/playground/run` is answering `503 playground_unavailable`; the registry itself is unaffected, which is why this is a warning and why the sandbox is deliberately absent from `/health/ready`. The API only reports what it can observe — whether the broker answers over its socket — so work outward from there. Check `systemctl status rux-playground` and `journalctl -u rux-playground --since "30 minutes ago" -o cat`, then `systemctl status docker`. A broker that will not start after a deploy is usually the release: `ConditionFileIsExecutable` keeps the unit down when the promoted release carries no `bin/rux-playgroundd`, which is expected on a rollback to an artifact built before the playground existed. Confirm the socket exists and is group-readable with `ls -l /run/rux-playground/run.sock` (`0660`, owned `rux-playground`), and that `rux-server` is still in the `rux-playground` group; `/run` is a tmpfs, so a missing directory after a reboot points at `/etc/tmpfiles.d/rux-playground.conf`. Verify the daemon answers with `sudo -u rux-playground docker version`. Never paste submitted source into a ticket. Resolve when the availability gauge reports 1 for five minutes.

## RuxServerPlaygroundElevatedFailures

Impact: a sustained share of runs is failing for reasons the service is answerable for, rather than because submitted code did not compile. Break `rux_playground_runs_total` down by `outcome` first, because the three contributing values mean different things. `unavailable` is the broker or the Docker daemon being unreachable — treat it as [RuxServerPlaygroundUnavailable](#ruxserverplaygroundunavailable). `timed_out` means runs are exceeding `RUX_PLAYGROUND_TIMEOUT_SECONDS`; check host CPU and memory pressure against the concurrency limit before raising any timeout, since the in-container `timeout(1)` is the primary bound and the outer deadline is only a backstop. `internal` means a malformed frame or a broker fault, and is the one to escalate with logs. Check whether the sandbox image is still present (`docker image inspect rux-playground:<version>`), since an absent image fails every run. Reducing `RUX_RATE_LIMIT_PLAYGROUND_PER_MINUTE` or `RUX_PLAYGROUND_MAX_CONCURRENCY` sheds load without touching the registry; setting `RUX_PLAYGROUND_ENABLED=false` removes the endpoint entirely and is the safe last resort. Resolve after the fifteen-minute ratio stays below threshold.
